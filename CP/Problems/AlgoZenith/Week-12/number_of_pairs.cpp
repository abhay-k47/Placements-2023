...
string a;
int len;
ll dp[52][2][2][2][512][512];

ll rec(int idx, int xhi, int yhi, int comp, int xsum, int ysum)
{
  if (idx == len)
  {
    if (comp && xsum < ysum && primes.count(xsum + ysum))
      return 1;
    else
      return 0;
  }
  ll& ans = dp[idx][xhi][yhi][comp][xsum][ysum];
  if (ans == -1)
  {
    ans = 0;
    int xl = 0, xr = 9;
    ...
    for (int i = xl; i <= xr; i++)
    {
      for (int j = yl; j <= yr; j++)
      {
        ...
        if (comp == 0 && i < j)
          ncomp = 1;
        (ans += rec(idx + 1, nxhi, nyhi, ncomp, xsum + i, ysum + j)) %= MOD;
      }
    }
  }
  return ans;
