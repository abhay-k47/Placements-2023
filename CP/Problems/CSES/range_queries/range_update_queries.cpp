...
public:
  ...
  {
    return queryUtil(root, 0, n - 1, l, r).val;
  }

  ll query(int idx)
  {
    ll ans = 0;
    for (int i = n + idx; i >= 1; i /= 2)
      ans += tree[i].val;
    return ans;
  }
...
private:
  ...
  void updateUtil(int node, int low, int high, int l, int r, int x)
  {
    if (l <= low && high <= r)
    {
      tree[node].val += x;
    ...
    if (high < l || r < low)
      return;
    int mid = low + (high - low) / 2;
    updateUtil(2 * node, low, mid, l, r, x);
    updateUtil(2 * node + 1, mid + 1, high, l, r, x);
  }
};
...
void solve()
{
  ...
  while (q--)
  {
    ...
    {
      cin >> k;
      cout << v[k - 1] + stree.query(k - 1) << endl;
    }
  }
}
