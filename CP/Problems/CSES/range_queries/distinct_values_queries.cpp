...
public:
  ...
  {
    return queryUtil(root, 0, n - 1, l, r).s.size();
  }
...
private:
  ...
  {
    Node result;
    result.s.insert(all(a.s));
    result.s.insert(all(b.s));
    return result;
  }
  ...
  Node queryUtil(int node, int low, int high, int l, int r)
  {
    ...
    auto left = queryUtil(2 * node, low, mid, l, r), right = queryUtil(2 * node + 1, mid + 1, high, l, r);
    if (left.s.size() < right.s.size())
      swap(left.s, right.s);
    left.s.insert(right.s.begin(), right.s.end());
    return left;
  }
};
