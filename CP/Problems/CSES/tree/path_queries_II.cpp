...
public:
  ...
  {
    return queryUtil(root, 0, n - 1, l, r).mx;
  }
  ...
  friend ostream& operator<<(ostream& out, const SegmentTree& stree)
  {
    int i = 1;
    for (int i = 1; i < 2 * stree.n; i++)
    {
      out << stree.tree[i].mx << " ";
    }
    cout << endl;
...
void solve()
{
  ...
  while (q--)
  {
    ...
    if (op == 1)
    {
      ...
      stree.update(tout[s], -x);
    }
    else if (op == 2)
    {
      cin >> a >> b;
      cout << stree.query(tin[root], tin[s]) << endl;
    }
  }
}
