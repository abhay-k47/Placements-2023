...
void solve()
{
  ...
  /**
   *  dp[i][0] = ways to build tower of height i with two tile of width 1 at bottom
   *  dp[i][1] = ways to build tower of height i with a tile of width 2 at bottom
   *
   *
   */
  dp[1][0] = dp[1][1] = 1;
  for (int i = 2; i <= n; i++)
    dp[i][0] = ((dp[i - 1][0] + dp[i - 1][1]) + 2 * dp[i - 1][0] + dp[i - 1][0]) % MOD,
