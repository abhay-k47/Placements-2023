...
int rec(int i, int j, int mask)
{
  ...
  {
    ans = 0;
    int up = mask & 1, left = mask & (1 << (cols - 1)), ni = i + (j + 1) / cols, nj = (j + 1) % cols;
    if (i && !up)
      /** put vertical tile */
      (ans += rec(ni, nj, (mask >> 1) | (1 << (cols - 1)))) %= MOD;
    else
    {
      /** don't put tile */
      (ans += rec(ni, nj, (mask >> 1))) %= MOD;
      if (cols >= 2 && j && !left)
        /** put horizontal tile */
        (ans += rec(ni, nj, (mask >> 1) | (1 << (cols - 1)) | (1 << (cols - 2)))) %= MOD;
    }
  }
  return ans;
}
