...
ll rec(int idx, bool lo, bool hi, int last, bool zero)
{
  //   cout << idx << " " << lo << " " << hi << " " << last << " " << zero << endl;
  if (idx == len)
  {
    if (last == -1)
      return 0;
  ...
  if (last == -1 || dp[idx][lo][hi][last][zero] == -1)
  {
    ll ans = 0;
    int l = 0;
    int r = 9;
    ...
    for (int i = l; i <= r; i++)
    {
      if (i != last || zero)
      {
        ...
        if (i != 0)
          nzero = false;
        ans += rec(idx + 1, nlo, nhi, i, nzero);
      }
    }
    dp[idx][lo][hi][last][zero] = ans;
  }
  return dp[idx][lo][hi][last][zero];
}
void solve()
{
  cin >> a >> b;
