...
void solve()
{
  ...
  for (int i = 0; i < topo_order.size(); i++)
    pos[topo_order[i]] = i;
  vi dp(topo_order.size(), 1);
  vi p(topo_order.size(), -1);
  for (int i = pos[1]; i < topo_order.size(); i++)
    for(int v: g[topo_order[i]])
        if(dp[pos[v]] < dp[i] + 1)
          dp[pos[v]] = dp[i] + 1, p[pos[v]] = i;
    vi ans;
    int curr = pos[n];
    while (p[curr] != -1)
    {
      ans.pb(topo_order[curr]);
      curr = p[curr];
    }
    ans.pb(topo_order[curr]);
    reverse(all(ans));
    cout << dp[pos[n]]  << endl << ans << endl;
}
